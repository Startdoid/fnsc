...
* Attributes:
*  crud - операция выполняемая на клиенте под управлением webix - это insert, update, delete
*  parent - родитель ветви дерева, если передан ноль то ветвь является корнем дерева
*  id - сегмента (в данном случае сегментом является пользователь), относительно которого извлекается массив групп
*  - если значение 0, то извлекаются все группы в соответствии с настройками приватности для групп
*  т.е. все "публичные" группы + группы которые видимы осн-пользователю
* Result:
*****************************************************************************/
