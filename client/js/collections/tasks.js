collectionTasks = Backbone.Collection.extend({
  ...
  newTask: function(parentId) {
    ...
    var newModel = new App.Models.Task({ 
      id:newId, 
      parent_id:parentId,
  ...
  //удаляет указанную задачу из коллекции и все вложенные задачи
  //selectedId - id задачи, которую необходимо удалить
  removeTask: function(selectedId) {
    var listTask = this.getInnerTask(selectedId);
    
    for (var i = listTask.length - 1; i >= 0; i--) {
      this.remove(listTask[i]);
    }
  },
  moveTask: function(selectedId, direction, indexDestination, parentDestination)
  {
    ...
    var deletedElements = this.models.splice(currentElementIndex, listTask.length);
